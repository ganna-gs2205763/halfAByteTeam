...
class GenerateStudyPlanScreen extends ConsumerStatefulWidget {
  final String stId;
  const GenerateStudyPlanScreen({super.key, required this.stId});
  ...
  Future<void> generatePlan(List<Competency> competencies) async {
    ...
    try {
      ...
      String prompt =
          ...
          "Allocate more time for complex topics and keep the plan to a maximum of 10 weeks."
          "in the different weeks, but not in more that 3 different weeks."
          ...
          "- topic xyz"
          "- topic xyz"
          "and so on. Do not add any extra text or explanation to your response";
      ...
      for (Competency comp in competencies) {
        compProgressNotifier.addCompProgress(CompetencyProgress(
            stId: student.stId, compId: comp.compId, done: false));
      }
      ...
      //     pathParameters: {'stId': student.stId, 'studyPlan': strResponse});
    } catch (error) {
      context.pop();
